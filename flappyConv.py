...
def neural_network_model(input_size, input_size2):
    ...
    network = conv_2d(network, 128, 7, activation='relu', strides=1)
    network = conv_2d(network, 256, 9, activation='relu', strides=1)
...
def train_model(training_data, model=False):
    ...
    X = np.array([i[2] for i in training_data]).reshape(-1, 80, 80, 1)
...
#training_data = np.load('flappy6.npy')
model = train_model(training_data)
model.save('flappyConv.model')


#model = neural_network_model(80 ,80)
#model.load('flappy3.model')
scores = []
accepted_scores = []
choices = []
for each_game in range(100):
    ...
    for _ in range(goal_steps):
        ...
        if len(prev_obs) == 0:
            ...
            action = model.predict(prev_obs.reshape(-1, 80, 80, 1))[0]
